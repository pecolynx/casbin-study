...
func Test_object_hierarchy_keymatch(t *testing.T) {
	const conf = `
	[request_definition]
	r = sub, obj, act
	...
	[matchers]
	m = g(r.sub, p.sub) && (keyMatch(r.obj, p.obj) || g2(r.obj, p.obj)) && r.act == p.act
	`
	...
	tests := []test_soa{
		...
		{subject: "charlie", object: "data2", action: "read", want: true},
		{subject: "charlie", object: "data_parent", action: "read", want: true},
	}
